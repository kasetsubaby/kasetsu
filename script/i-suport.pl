...
sub CHARA_MAIN_INPUT {
	...
	if($REFREE){
		...
		if($r_url ne $SANGOKU_URL){ &ERR2("ERR No.002<BR>そのキャラクターは作れません。"); }
	}

	@NEW_DATA=();
	unshift(@NEW_DATA,"$kid<>$kpass<>$kname<>$kchara<>$kstr<>$kint<>$klea<>$kcha<>$ksol<>$kgat<>$kcon<>$kgold<>$krice<>$kcex<>$kclass<>$karm<>$kbook<>$kbank<>$ksub1<>$ksub2<>$kpos<>$kmes<>$host<>$kdate<>$kmail<>$kos<>\n");
...
print "<form action=\"i-status.cgi\" method=\"post\"><input type=hidden name=id value=$kid><input type=hidden name=pass value=$kpass><input type=hidden name=mode value=STATUS><input type=submit value=\"街へ戻る\"></form>";
