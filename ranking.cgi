...
sub RANKING {
	...
	($kid,$kpass,$kname,$kchara,$kstr,$kint,$klea,$kcha,$ksol,$kgat,$kcon,$kgold,$krice,$kcex,$kclass,$karm,$kbook,$kbank,$ksub1,$ksub2,$kpos,$kmes,$khost,$kdate,$kmail,$kos) = split(/<>/);
		...
		for($j=0;$j<$country_no;$j++){
			if($kcon eq "$c_no[$j]"){
				...
				$list[$j] .= "<TR><TD><img src=$IMG/$kchara.gif></TD><TD><a href=ranking6.cgi?id= target=_blank>$kname</a></TD><TD>$kstr</TD><TD>$kint</TD><TD>$klea</TD><TD>$klea</TD><TD>$LANK[$s_num]</TD><TD>$ITEM_NO[0]</TD><TD>$rmターン</TD></TR>";
				}else{
	...
	&HEADER;
	$l_rank = "<TR><TD></TD><TH>名前</TH><TH>武力</TH><TH>知力</TH><TH>統率力</TH><TH>人望</TH><TH>階級</TH><TH>装備</TH><TH>削除まで</TH></TR>";
...
<TR><TH colspan=2 bgcolor=$ELE_C[$xele]><font color=$ELE_BG[$xele]>君主</TH><TH bgcolor=FFFFFF><font size=3 color=$ELE_BG[$xele]>$c_k_name[$c_c]</TH><TD bgcolor=$ELE_C[$xele]><font color=$ELE_BG[$xele]>武将数</TD><TD bgcolor=$ELE_C[$xele]><font color=$ELE_BG[$xele]> $c[$c_c] 名</TD><TD bgcolor=$ELE_C[$xele]><a href=./$FILE_RANK?mode=C_RAN&con_no=$xcid>武将一覧</a></TD></TR>
<TR><TH bgcolor=$ELE_BG[$xele]><font color=$ELE_C[$xele]>軍師</TH><TH bgcolor=$ELE_C[$xele]><font size=2 color=$ELE_BG[$xele]>$c_g_name[$c_c]</TH><TH bgcolor=$ELE_BG[$xele]><font color=$ELE_C[$xele]>軍師補佐</TH><TH bgcolor=$ELE_C[$xele]><font size=2 color=$ELE_BG[$xele]>$c_d_name[$c_c]</TH><TH bgcolor=$ELE_BG[$xele]><font color=$ELE_C[$xele]>大将軍</TH><TH bgcolor=$ELE_C[$xele]><font size=2 color=$ELE_BG[$xele]>$c_d_name[$c_c]</TH></TR>
<TR><TD bgcolor=$ELE_C[$xele]><font color=$ELE_BG[$xele]>支配都市数</TD><TD colspan=5 bgcolor=$ELE_C[$xele]><font color=$ELE_BG[$xele]>$town_c[$c_c]都市($mes[$c_c])</TD></TR></TBODY></TABLE><BR>
...
sub C_RAN{
	...
	($kid,$kpass,$kname,$kchara,$kstr,$kint,$klea,$kcha,$ksol,$kgat,$kcon,$kgold,$krice,$kcex,$kclass,$karm,$kbook,$kbank,$ksub1,$ksub2,$kpos,$kmes,$khost,$kdate,$kmail,$kos) = split(/<>/);
		...
		for($j=0;$j<$country_no;$j++){
			...
			$list[$j] .= "<TR><TD><img src=$IMG/$kchara.gif></TD><TD><a href= target=_blank>$kname</a></TD><TD>$kstr</TD><TD>$kint</TD><TD>$klea</TD><TD>$ksol</TD><TD>$LAMK[$s_num]</TD><TD>$ITEM_NO[0]</TD><TD>$rmターン</TD></TR>";
			$c[$j]++;
			}
	...
	($kid,$kpass,$kname,$kchara,$kstr,$kint,$klea,$kcha,$ksol,$kgat,$kcon,$kgold,$krice,$kcex,$kclass,$karm,$kbook,$kbank,$ksub1,$ksub2,$kpos,$kmes,$khost,$kdate,$kmail,$kos) = split(/<>/);
			...
			$list[0] .= "<TR><TD><img src=$IMG/$kchara.gif></TD><TD><a href= target=_blank>$kname</a></TD><TD>$kstr</TD><TD>$kint</TD><TD>$klea</TD><TD>$ksol</TD><TD></TD><TD>$rmターン</TD></TR>";
			}
				$c[0]++;
			}
		}
